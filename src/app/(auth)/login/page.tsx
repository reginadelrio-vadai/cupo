  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) {
      if (error.message.includes('Email not confirmed')) {
        setError('Tu email no ha sido confirmado. Revisa tu bandeja de entrada o regístrate de nuevo.')
      ...
      <Card className="w-full max-w-[400px] border-[0.5px] border-[#E2E8F0]">
        ...
        <CardContent>
          ...
          <p className="mt-4 text-center text-sm text-[#475569]">
            ¿No tienes cuenta?{' '}
            <Link href="/register" className="text-[#00B8E6] hover:opacity-80 transition-opacity duration-150">
              Regístrate
            </Link>
          </p>
        </CardContent>
